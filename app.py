...
#Query Gorrilla Server

def gorilla_query(prompt,model):
    try:
        completion=openai.ChatCompletion.create(
            model=model,
            messages=[{'role':'user','content':prompt}]
        )
        print('Responce:',completion)
        ...
        if len(input_prompt) > 0:
            ...
            with col1:
                if option == "gorilla-7b-hf-v1":
                    result = gorilla_query(prompt=input_prompt, model=option)
                    st.write(result)
